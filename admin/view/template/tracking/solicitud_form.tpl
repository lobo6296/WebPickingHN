  ...
  <div class="page-header">
    <div class="container-fluid">
	  ...
	  <?php
	       if ($MRHW_ESTADO==0) {
	  ?>		
        <button type="submit" form="formid" data-toggle="tooltip" title="<?php echo $button_finish; ?>" class="btn btn-danger"><i class="fa fa-envelope"></i></button>
		<?php
		   }
	  ?>
	  </div>
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        ...
        <div class="form-row">
          ...
          <div class="row">
			...
			<div class="col-sm-2">			
				<div class="form-group">
					...
					<select name="emprecibe" id="emprecibe" class="form-control" 
					<?php echo ($MRHW_ESTADO==1)?' disabled':''; ?>
					/> 
					<option value="*" disabled selected>--Seleccione--</option>
					<?php  
							foreach ($empresas as $value) { ?> 
						<?php if ($value['valor'] == $filter_empresa) { ?> 
						<option value="<?php echo $value['valor']; ?>" selected="selected"><?php echo $value['nombre']; ?>
						</option> 
						<?php } 
						else { ?> 
						<option value="<?php echo $value['valor']; ?>"><?php echo $value['nombre']; ?></option> <?php } ?> 
						<?php } ?> 
						</select>
				</div>						
			...
			<div class="col-sm-2">			
					<div class="form-group">
						<label for="input-rtn"><?php echo $entry_rtn_emp; ?></label>
						<input type="text" name="rtnemp" style="text-transform:uppercase;" value="<?php echo $RTNEMP; ?>" placeholder="<?php echo $entry_rtn_emp; ?>" id="rtnemp" class="form-control form-control-sm" 
						<?php echo ($MRHW_ESTADO==1)?' disabled':''; ?>
						/>     							
					</div>			
			</div>		
          ...
          <div class="col-sm-2">			
					<div class="form-group">
						<label for="input-rtn"><?php echo $entry_rtn; ?></label>
						<input type="text" name="rtn" style="text-transform:uppercase;" value="<?php echo $RTN; ?>" placeholder="<?php echo $entry_rtn; ?>" id="rtn" class="form-control form-control-sm" 
						<?php echo ($MRHW_ESTADO==1)?' disabled':''; ?>
						/>     							
					</div>			
			</div>		
		  ...
		  <div class="col-sm-2">			
				<div class="form-group">
					<label for="input-vehiculo"><?php echo $entry_vehiculo; ?></label>
					...
					<?php  
						...
						<?php if ($value['valor'] == $filter_vehiculos) { ?> 
						<option value="<?php echo $value['valor']; ?>" selected="selected"><?php echo $value['nombre']; ?>
						</option> 
						<?php } 
						else { ?> 
			...
			<div class="col-sm-2">			
					<div class="form-group">
						<label for="input-placa"><?php echo $entry_placa; ?></label>
						<input type="text" name="placa" style="text-transform:uppercase;" value="<?php echo $PLACA; ?>" placeholder="<?php echo $entry_placa; ?>" id="placa" class="form-control form-control-sm" 
						<?php echo ($MRHW_ESTADO==1)?' disabled':''; ?>
						/>     							
					</div>			
			</div>
		  ...
		  <div class="col-sm-3">
                        ...
                        <input type="text" name="hwfechaentrega" value="<?php echo $HWFECHAENTREGA; ?>" placeholder="<?php echo $entry_hwfechaentrega; ?>" data-date-format="DD-MM-YYYY hh:mm" id="hwfechaentrega" class="form-control" 
						<?php echo ($MRHW_ESTADO==1)?' disabled':''; ?>
						/>
              ...
              <div class="table-responsive">
				...
				<tbody>
					<tr id="registro">
						...
						<td id="hwartcod_des" class="text-left" style="width:20%"></td>
						<td class="text-right">
                ...
                <table class="table table-bordered">
                  ...
                  <tbody id="detalle">
                    ...
                    <tr>
					  ...
					  <td class="text-left"><button type="button" onclick="elimina(<?php echo $order_product['HWLINSOL']; ?>);" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>
...
$(document).ready(function() {
    ...
    $("#formid").validate({
        ...
        submitHandler: function(form){
            ...
            $.ajax({
                type: "POST",
                url : "index.php?route=tracking/solicitud/validarSolicitud&token=<?php echo $token; ?>&hwmr=" + hwmr,
                data: dataString,
...
function cargaArticulos(x){
	...
	var subcuenta = document.getElementById("registro[subcuenta]").value;
    var html ='';
	var row  = 0;
	...
	hwartcod = hwartcod.toUpperCase();
		...
		$.ajax({
			...
			type: 'POST',
			data: {hwartcod,hwsolest,subcuenta},
			dataType: "json",
			success: function(data){
				$("#oculto").html('');
...
function elimina(detalle_row) {
	var linea = detalle_row;
	var hwmr		= document.getElementById("hwmr").value;
	var hwsolest    = document.getElementById("hwsolest").value;
	
	$.ajax({
		url: 'index.php?route=tracking/solicitud/putDetalle&token=<?php echo $token; ?>&hwmr=' + hwmr + '&linea=' + linea + '&hwsolest=' + hwsolest,
		dataType: 'json',	
	});
	
	//$('#detalle-row' + detalle_row).remove();
	refrescar();
}
...
function validar() {
	...
	var rtnemp			= document.getElementById("rtnemp").value;
	var perrecibe 		= document.getElementById("perrecibe").value;
	var empresa 		= document.getElementById("emprecibe").value;
		...
		if (validar==1) {
			if (hwartcod&&hwsolcant) {
				...
				$.ajax({
					...
					'&autonombre=' + autonombre + '&placa=' + placa + '&hwtipsol=' + hwtipsol + '&hwfechaentrega=' + hwfechaentrega + '&rtn=' + rtn + '&perrecibe=' + perrecibe + '&empresa=' + empresa + '&vehiculo=' + vehiculo + '&rtnemp=' + rtnemp,
					dataType: 'json',
...
$('input[name=\'registro[serie]\']').autocomplete({
	'source': function(request, response) {
		var subcuenta = document.getElementById("registro[subcuenta]").value;
		$.ajax({
			url: 'index.php?route=tracking/solicitud/llenaSerie&token=<?php echo $token; ?>&serie=' + request + '&subcuenta=' + subcuenta,
			dataType: 'json',
	...
	'select': function(item) {
		$('input[name=\'registro[serie]\']').val(item['label']);
		//activaInput(false)
	},
	'focus':  function() {
	$(this).keydown();
...
/*$('input[name=\'registro[rtn]\']').autocomplete({
	'source': function(request, response){
		$.ajax({
			url: 'index.php?route=tracking/solicitud/llenaSitio&token=<?php echo $token; ?>'
		})
	}
});*/
...
$('input[name=\'registro[hwartcod]\']').autocomplete({
    'source': function(request, response) {
		var subcuenta = document.getElementById("registro[subcuenta]").value;
		var serie = document.getElementById("registro[serie]").value;
	  ...
	  */
      $.ajax({
        url: 'index.php?route=tracking/solicitud/llenaArt&token=<?php echo $token; ?>&hwartcod=' + request + '&subcuenta=' + subcuenta + '&serie=' + serie,
        dataType: 'json',
        success: function(json) {
...
$('.date').datetimepicker({
	...
	pickTime: false,
	minDate: da
	//minDate:"<?php echo $fecha_inicial; ?>"
}); 
--></script>
<?php echo $footer; ?>