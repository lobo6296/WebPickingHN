<?php
class ControllerReportoutbounds extends Controller {
	...
	public function download() {
		...
		$filter_data = array(
		 ...
		 $this->response->redirect($this->url->link('report/outbounds', 'token=' . $this->session->data['token'] . $url, true));
		}
		$this->index();		
	}
	...
	public function index() {
		...
		$data['pdf'] = $this->url->link('report/outbounds/pdf', 'token=' . $this->session->data['token'].$url, 'SSL');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('report/outbounds', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$stock_total = $this->model_report_tigo->getTotalOutbounds($filter_data);

		$results = $this->model_report_tigo->getOutbounds($filter_data);
		...
		$data['export'] = $this->url->link('report/outbounds/download', 'token=' . $this->session->data['token'], true);
		
		$data['excel'] = $this->url->link('report/outbounds/excel', 'token=' . $this->session->data['token'], 'SSL');
		...
		$pagination->url = $this->url->link('report/outbounds', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        /*
		if ($pdf) {
          $this->response->setOutput(pdf($this->load->view('report/outbounds.tpl', $data),$data));
		} else {
		*/	
		  $this->response->setOutput($this->load->view('report/outbounds.tpl', $data));
		/*
		}
		*/
	}
