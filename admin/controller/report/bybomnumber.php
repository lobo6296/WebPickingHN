<?php
class ControllerReportbybomnumber extends Controller {
	...
	public function download() {
		...
		$filter_data = array(
		 ...
		 $this->response->redirect($this->url->link('report/bybomnumber', 'token=' . $this->session->data['token'] . $url, true));
		}
		$this->index();		
	}
	...
	public function index() {
		...
		$data['pdf'] = $this->url->link('report/bybomnumber/pdf', 'token=' . $this->session->data['token'].$url, 'SSL');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('report/bybomnumber', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$stock_total = $this->model_report_tigo->getTotalbyBOMNumber($filter_data);

		$results = $this->model_report_tigo->getbyBOMNumber($filter_data);
		...
		$data['export'] = $this->url->link('report/bybomnumber/download', 'token=' . $this->session->data['token'], true);
		
		$data['excel'] = $this->url->link('report/bybomnumber/excel', 'token=' . $this->session->data['token'], 'SSL');
		...
		$pagination->url = $this->url->link('report/bybomnumber', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        /*
		if ($pdf) {
          $this->response->setOutput(pdf($this->load->view('report/bybomnumber.tpl', $data),$data));
		} else {
		*/	
		  $this->response->setOutput($this->load->view('report/bybomnumber.tpl', $data));
		/*
		}
		*/
	}
