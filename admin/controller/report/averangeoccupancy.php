<?php
class ControllerReportaverangeoccupancy extends Controller {
	...
	public function download() {
		...
		$filter_data = array(
			...
			'titulo'            => 'Reporte Diario Ocupaciones por Carril',
			'reporte'           => 'averangeoccupancy'
		);
		 ...
		 $this->response->redirect($this->url->link('report/averangeoccupancy', 'token=' . $this->session->data['token'] . $url, true));
		}
		$this->index();		
	}
	...
	public function index() {
		...
		$data['pdf'] = $this->url->link('report/averangeoccupancy/pdf', 'token=' . $this->session->data['token'].$url, 'SSL');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('report/averangeoccupancy', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$stock_total = $this->model_report_tigo->getTotalAverangeoccupancy($filter_data);

		$results = $this->model_report_tigo->getAverangeoccupancy($filter_data);

		foreach ($results as $result) {
			$data['stock'][] = array(
				'tgfecha'		=> $result['TGFECHA'],
				'tgcarril'     	=> $result['TGCARRIL'],
				'tgancho'    	=> $result['TGANCHO'],
				'tglargo'   	=> $result['TGLARGO'],
				'porcarril' 	=> $result['PORCARRIL']
			);
		}
		...
		$data['column_tgfecha']		= $this->language->get('column_tgfecha');
		$data['column_tgcarril']	= $this->language->get('column_tgcarril');
		$data['column_tgancho']		= $this->language->get('column_tgancho');
		$data['column_tglargo']   	= $this->language->get('column_tglargo');
		$data['column_porcarril'] 	= $this->language->get('column_porcarril');
		...
		$data['export'] = $this->url->link('report/averangeoccupancy/download', 'token=' . $this->session->data['token'], true);
		
		$data['excel'] = $this->url->link('report/averangeoccupancy/excel', 'token=' . $this->session->data['token'], 'SSL');
		...
		$pagination->url = $this->url->link('report/averangeoccupancy', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        /*
		if ($pdf) {
          $this->response->setOutput(pdf($this->load->view('report/averangeoccupancy.tpl', $data),$data));
		} else {
		*/	
		  $this->response->setOutput($this->load->view('report/averangeoccupancy.tpl', $data));
		/*
		}
		*/
	}
