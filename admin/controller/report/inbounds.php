...
class ControllerReportinbounds extends Controller {
	...
	public function download() {
		...
		$filter_data = array(
		 ...
		 $this->response->redirect($this->url->link('report/inbounds', 'token=' . $this->session->data['token'] . $url, true));
		}
		$this->index();		
	}
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('report/inbounds', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$stock_total = $this->model_report_tigo->getTotalInbounds($filter_data);

		$results = $this->model_report_tigo->getInbounds($filter_data);
		...
		$data['export'] = $this->url->link('report/inbounds/download', 'token=' . $this->session->data['token'], true);
		
		$data['excel'] = $this->url->link('report/inbounds/excel', 'token=' . $this->session->data['token'], 'SSL');
		...
		$pagination->url = $this->url->link('report/inbounds', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        /*
		if ($pdf) {
          $this->response->setOutput(pdf($this->load->view('report/inbounds.tpl', $data),$data));
		} else {
		*/	
		  $this->response->setOutput($this->load->view('report/inbounds.tpl', $data));
		/*
		}
		*/
	}
