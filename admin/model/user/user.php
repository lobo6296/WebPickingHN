...
class ModelUserUser extends Model {
	public function addUser($data) {
		$this->mysql->query("INSERT INTO `user`
		                        SET username = '" . $this->mysql->escape($data['username']) . "'
								   ,user_group_id = '" . (int)$data['user_group_id'] . "'
								   ,salt = '" . $this->mysql->escape($salt = token(9)) . "'
								   ,password = '" . $this->mysql->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "'
								   ,firstname = '" . $this->mysql->escape($data['firstname']) . "'
								   ,lastname = '" . $this->mysql->escape($data['lastname']) . "'
								   ,email = '" . $this->mysql->escape($data['email']) . "'
								   ,image = '" . $this->mysql->escape($data['image']) . "'
	...
	public function editUser($user_id, $data) {
		...
		if ($data['password']) {
			$this->mysql->query("UPDATE `user`
			                        SET salt     = '" . $this->mysql->escape($salt = token(9)) . "'
									   ,password = '" . $this->mysql->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "' 
							      WHERE user_id = '" . (int)$user_id . "'");
		}
	}
	...
	public function getUser($user_id) {
		$query = $this->mysql->query("SELECT *, (SELECT ug.name FROM `user_group`ug WHERE ug.user_group_id = u.user_group_id) AS user_group FROM `user`u WHERE u.user_id = '" . (int)$user_id . "'");
	...
	public function getUserByEmail($email) {
		$query = $this->mysql->query("SELECT DISTINCT * FROM `user`WHERE LCASE(email) = '" . $this->mysql->escape(utf8_strtolower($email)) . "'");
	...
	public function getUsers($data = array()) {
		$sql = "select u.user_id,u.username,ug.name,u.status,u.date_added
                  from `user` u
                      ,user_group ug
