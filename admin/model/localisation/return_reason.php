...
class ModelLocalisationReturnReason extends Model {
	public function addReturnReason($data) {
		foreach ($data['return_reason'] as $language_id => $value) {
			if (isset($return_reason_id)) {
				$this->mysql->query("INSERT INTO  return_reason SET return_reason_id = '" . (int)$return_reason_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->mysql->escape($value['name']) . "'");
			} else {
				$this->mysql->query("INSERT INTO  return_reason SET language_id = '" . (int)$language_id . "', name = '" . $this->mysql->escape($value['name']) . "'");

				$return_reason_id = $this->mysql->getLastId();
			}
		}
	...
	public function editReturnReason($return_reason_id, $data) {
		$this->mysql->query("DELETE FROM  return_reason WHERE return_reason_id = '" . (int)$return_reason_id . "'");
	...
	public function getReturnReason($return_reason_id) {
		$query = $this->mysql->query("SELECT * FROM  return_reason WHERE return_reason_id = '" . (int)$return_reason_id . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getReturnReasons($data = array()) {
		if ($data) {
			...
			if (!$return_reason_data) {
				$query = $this->mysql->query("SELECT return_reason_id, name FROM  return_reason WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY name");
	...
	public function getReturnReasonDescriptions($return_reason_id) {
		...
		$query = $this->mysql->query("SELECT * FROM  return_reason WHERE return_reason_id = '" . (int)$return_reason_id . "'");
	...
	public function getTotalReturnReasons() {
		$query = $this->mysql->query("SELECT COUNT(*) AS total FROM  return_reason WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");
