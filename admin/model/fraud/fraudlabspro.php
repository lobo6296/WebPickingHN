...
class ModelFraudFraudLabsPro extends Model {
	public function install() {
		$this->mysql->query("
			CREATE TABLE IF NOT EXISTS ` fraudlabspro` (
		...
		$this->mysql->query("INSERT INTO ` order_status` (`language_id`, `name`) VALUES (1, 'Fraud');");
		$status_fraud_id = $this->mysql->getLastId();

		$this->mysql->query("INSERT INTO ` order_status` (`language_id`, `name`) VALUES (1, 'Fraud Review');");
		$status_fraud_review_id = $this->mysql->getLastId();

		$this->mysql->query("INSERT INTO ` setting` (`code`, `key`, `value`, `serialized`) VALUES ('fraudlabspro', 'fraudlabspro_score', '80', '0');");
		$this->mysql->query("INSERT INTO ` setting` (`code`, `key`, `value`, `serialized`) VALUES ('fraudlabspro', 'fraudlabspro_order_status_id', '$status_fraud_id', '0');");
		$this->mysql->query("INSERT INTO ` setting` (`code`, `key`, `value`, `serialized`) VALUES ('fraudlabspro', 'fraudlabspro_review_status_id', '$status_fraud_review_id', '0');");
		$this->mysql->query("INSERT INTO ` setting` (`code`, `key`, `value`, `serialized`) VALUES ('fraudlabspro', 'fraudlabspro_approve_status_id', '2', '0');");
		$this->mysql->query("INSERT INTO ` setting` (`code`, `key`, `value`, `serialized`) VALUES ('fraudlabspro', 'fraudlabspro_reject_status_id', '8', '0');");

		$this->cache->delete('order_status.' . (int)$this->config->get('config_language_id'));
	}

	public function uninstall() {
		//$this->mysql->query("DROP TABLE IF EXISTS ` fraudlabspro`");
		$this->mysql->query("DELETE FROM ` order_status` WHERE `name`='Fraud'");
		$this->mysql->query("DELETE FROM ` order_status` WHERE `name`='Fraud Review'");
	}

	public function getOrder($order_id) {
		$query = $this->mysql->query("SELECT * FROM ` fraudlabspro` WHERE order_id = '" . (int)$order_id . "'");
